  ...
  return (
    ...
    >
      <div className="mx-auto max-w-[1280px] px-6 lg:px-12 py-20 lg:py-28">
        ...
        <h2 id="why-me-heading" className="font-display text-4xl lg:text-6xl leading-[1.02] tracking-tight max-w-4xl">
          Why organizations work with me
        </h2>
        <p className="mt-8 max-w-3xl text-lg opacity-75 leading-relaxed">
          Most AI transformation programs focus on technology selection and strategy decks. I focus on
          execution — because technology rarely fails. <AccentText className="opacity-100">Operating models do.</AccentText>
        </p>
        <p className="mt-6 max-w-3xl text-lg opacity-75 leading-relaxed">
          My perspective is uncommon: deep-tech engineering and semiconductor R&D, followed by enterprise
          transformation leadership, AI enablement at scale, governance design, and measurement systems.
          I have built silicon and built the operating models that help 74,000-person organizations adopt AI.
        </p>

        <div className="mt-16 grid sm:grid-cols-2 lg:grid-cols-3 gap-10 lg:gap-12">
          {differentiators.map((item) => (
            <article key={item.title}>
              <h3 className="font-display text-2xl lg:text-3xl mb-4 opacity-95 leading-tight">{item.title}</h3>
        ...
        <blockquote className="mt-16 lg:mt-20 pt-12 border-t border-white/10">
          <p className="font-display text-2xl lg:text-3xl leading-[1.25] tracking-tight opacity-90 max-w-4xl italic">
            &ldquo;The organizations that win at AI are not the ones with the best models. They are the ones
            with the best <AccentText className="opacity-100 not-italic">operating models</AccentText>.&rdquo;
          </p>
        </blockquote>
      </div>
    </section>
